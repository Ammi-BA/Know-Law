...
structural and legal requirements of Egyptian Civil Code, including
...
class ContractValidator:
    ...
    def verify_signature(self, contract_text, signature=None, public_key_pem=None):
        """
        Verifies if the contract contains a signature block and optionally
        checks if the digital signature matches using RSA.
        """
        errors = []
        if not re.search(r'(توقيع|يمضي|إمضاء)', contract_text):
            errors.append("Missing Signature Block (التوقيع)")
        elif signature and public_key_pem:
            try:
                key = RSA.import_key(public_key_pem)
                h = SHA256.new(contract_text.encode())
                pkcs1_15.new(key).verify(h, bytes.fromhex(signature))
            except (ValueError, TypeError):
                errors.append("Invalid digital signature")
        return errors

    def process_ai_output(self, ai_generated_text, expected_category, signature=None, public_key_pem=None):
        ...
        signature_errors = self.verify_signature(ai_generated_text, signature, public_key_pem)

        all_errors = structural_errors + signature_errors
        is_valid = len(all_errors) == 0 and len(legal_warnings) == 0
        return is_valid, all_errors, legal_warnings
...
if __name__ == "__main__":
    ...
    # Sign the contract text
    h = SHA256.new(test_contract.encode())
    signature = pkcs1_15.new(key).sign(h).hex()

    # Validate
    is_valid, errs, warns = validator.process_ai_output(test_contract, "car_sale", signature, public_key)
